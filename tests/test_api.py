...
class TestCompaniesEndpoint:
    ...
    def test_rated_companies_are_listed(self, client, sample_entity):
        ...
        assert len(data) == 1
        assert data[0]["entity_name"] == sample_entity.entity_name
    ...
    def test_company_can_be_looked_up_by_id(self, client, sample_entity):
        ...
        assert resp.json()["entity_name"] == sample_entity.entity_name
    ...
    def test_compare_rejects_date_before_earliest_snapshot(
        self, client, sample_snapshot
    ):
        resp = client.get(
            "/companies/compare",
...
class TestUploadsEndpoint:
    def test_uploaded_files_are_listed(self, client, sample_file_meta):
        data = client.get("/uploads/").json()
        assert len(data) == 1
        assert data[0]["fname"] == sample_file_meta.fname
    ...
    def test_uploaded_file_can_be_retrieved_by_id(self, client, sample_file_meta):
        ...
        assert data["fname"] == sample_file_meta.fname
    ...
    def test_upload_count_is_tracked(self, client, sample_file_meta):
        stats = client.get("/uploads/stats").json()
        assert stats["total_uploads"] == 1
        assert stats["earliest_upload"] == sample_file_meta.ctime.isoformat()
